...
def main():
   ...
   train_value, train_label =file2matrix('./gaze_data/experiment_2/train/train.txt',8,1,8,0)
   val_value, val_label =file2matrix('./gaze_data/experiment_2/val/val.txt',8,1,8,0)
   

#    train_value = (train_value-np.mean(train_value))/np.std(train_value)
#    val_value = (val_value-np.mean(val_value))/np.std(val_value)
   ...
   val_dataloader = DataLoader(val_dataset, batch_size = 1024, shuffle=True, num_workers=2)
